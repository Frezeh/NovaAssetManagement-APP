...
import { fetchNmmf, addNmmfSale } from '../redux/ActionCreators';
...
export default function MoneyMarketFund() {
    ...
    const postNmmf = () => {        
        ...
        return fetch(baseUrl + 'stock/buy', {
          ...
          .then(stock => { dispatch(addNmmfSale(stock)); alert('Your Request Is Being Processed! '); push(); })
    ...
    const home = nmmf.nmmf.map((nmmf) => {
        return (
            <ScrollView key={nmmf._id}>
...
async function schedulePushNotification(date) {
    await Notifications.scheduleNotificationAsync({
      content: {
        ...
        body: 'Nova Money Market Fund Transaction Maturing ' + date + ' is in Progress',
        data: { data: 'goes here' },
      },
      trigger: { seconds: 2 },
    });
