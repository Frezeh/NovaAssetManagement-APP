...
import { fetchNdf, postNdfSale, addNdfSale } from '../redux/ActionCreators';
...
export default function NovaDollarFund() {
    ...
    const postNdf = () => {        
        ...
        return fetch(baseUrl + 'stock/buy', {
          ...
          .then(stock => { dispatch(addNdfSale(stock)); alert('Your Request Is Being Processed! '); push(); })
    ...
    const home = ndf.ndf.map((ndf) => {
        return (
            <ScrollView key={ndf._id}>
                <RenderCard item={ndf} />
            </ScrollView>
        );
    });
...
async function schedulePushNotification(date) {
    await Notifications.scheduleNotificationAsync({
      content: {
        ...
        body: 'Nova Dollar Fund Transaction Maturing ' + date + ' is in Progress',
        data: { data: 'goes here' },
      },
      trigger: { seconds: 2 },
    });
