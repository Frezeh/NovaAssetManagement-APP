...
//push the comment into the Redux store
export const addComment = (comment) => ({
  type: ActionTypes.ADD_COMMENT,
  payload: comment
});

//A function that creates an action object this addComment will take in four parameters; dish Id, rating, author and comment
...
export const loginUser = (creds) => (dispatch) => {
  ...
  return fetch(baseUrl + 'users/login', {
    ...
    //if response is correctly returned, response should contain the token here
    .then(response => {
      if (response.ok) {
        return response;
      }
...
export const postEtfSale = (value) => (dispatch) => {
  ...
  const bearer = AsyncStorage.getItem('token');//asyn
...
export const postNdfSale = (value) => (dispatch) => {
  ...
  const bearer = 'Bearer ' + AsyncStorage.getItem('token');//asyn
...
// export const postStockNmmf = (comment) => (dispatch) => {
...
//     .then(response => response.json())
//     //this response coming in from the server will contain the updated comment that has been posted to the server site
//     .then(response => dispatch(addNmmfSale(response)))
//     .catch(error => { alert('post sale', error.message); alert('Your sale could not be posted\nError: ' + error.message); });
// };
